...
export function AutonomousDashboard({ initialRuns, initialTopics }: Props) {
  ...
  const totals = useMemo(() => runs.reduce((a, r) => ({
    signals: a.signals + Number(r.signals_analyzed || 0), problems: a.problems + Number(r.problems_found || 0), opportunities: a.opportunities + Number(r.opportunities_found || 0),
  }), { signals: 0, problems: 0, opportunities: 0 }), [runs]);

  async function discover() {
    if (busy) return;
    setBusy(true); setError(''); setCompleted(0);
    try {
      for (let i = 0; i < CYCLES; i += 1) {
        ...
        setCompleted(i + 1); await refresh();
  ...
  return <section className="agent-panel">
    <div className="agent-hero">
      <div><div className="eyebrow">Autonomous opportunity engine</div><h2>Find product opportunities without choosing the topic.</h2><p>One click makes Soln-Agent choose fresh research directions, search public conversations, detect real problems, measure demand, check competition and turn the strongest signals into product opportunities.</p></div>
      <button className="discover-button" onClick={discover} disabled={busy}><span className="button-dot" />{busy ? 'Discovering…' : 'Discover opportunities'}</button>
    </div>
    <div className="source-strip"><span className="small">Live sources</span>{SOURCES.map(source => <span className="source-chip" key={source}><i />{source}</span>)}</div>
    ...
    <div className="opportunity-layout">
      <div className="glass-card opportunity-card"><div className="section-head"><div><div className="eyebrow">Output</div><h3>Product opportunities</h3></div><a className="link" href="/opportunities">View all →</a></div>
        {opportunities.length ? opportunities.slice(0, 6).map(o => <a className="opportunity-row" href={`/opportunities/${o.id}`} key={o.id}><div className="opp-rank">{Math.round(Number(o.score || 0))}</div><div className="opp-copy"><strong>{o.name}</strong><span>{o.problem?.target_customer || 'Customer segment being validated'}</span><small>{o.problem?.demand_score != null ? `Demand ${Math.round(o.problem.demand_score)}` : 'Demand analysis'} · {o.competitor_count ?? 0} competitors · {o.validation_count ?? 0} validation plan</small></div><span className="arrow">↗</span></a>) : <div className="empty-state"><div className="empty-orb">✦</div><strong>No opportunities yet</strong><span>Click Discover opportunities. The agent will decide what markets deserve investigation.</span></div>}
      </div>
      <div className="glass-card activity-card"><div className="section-head"><div><div className="eyebrow">Agent memory</div><h3>Research directions</h3></div></div>{topics.slice(0, 8).map(t => <div className="direction-row" key={t.id}><div><strong>{t.topic}</strong><span>Priority {t.priority} · researched {t.times_researched}×</span></div><em>{t.last_researched_at ? new Date(t.last_researched_at).toLocaleDateString() : 'Ready'}</em></div>)}{!topics.length && <div className="empty-state compact"><strong>Waiting for the first discovery pass.</strong></div>}</div>
    </div>
