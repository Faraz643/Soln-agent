...
export function AutonomousDashboard({ initialRuns, initialTopics }: Props) {
  ...
  async function runStep(jobId: string): Promise<boolean> {
    ...
    if (phase === 'discover') { setStage(j.source ? `Searching ${j.source} for real customer problems…` : 'Collecting evidence…'); setProgress(p => Math.min(48, p + 8)); }
    else if (phase === 'analyze') { setStage('Reading evidence and rejecting irrelevant noise…'); setProgress(p => Math.min(78, p + 5)); }
    else if (phase === 'cluster') { setStage('Grouping recurring problems and scoring product opportunities…'); setProgress(86); }
    else if (phase === 'finish') { setStage('Finalizing the strongest opportunities…'); setProgress(96); }
  ...
  async function discover() {
    if (busy) return; setBusy(true); setError(''); setProgress(3); setStage('Choosing a fresh research direction…');
    try {
      ...
      await refresh(); setStage('Discovery complete — new opportunities are ready.'); setProgress(100);
  ...
  return <section className="agent-panel">
    <div className="agent-hero"><div><div className="eyebrow">Autonomous opportunity engine</div><h2>Find product opportunities without choosing the topic.</h2><p>One click makes Soln-Agent choose a fresh direction, search public conversations across enabled sources, reject irrelevant noise, detect recurring problems, measure demand and rank product opportunities.</p></div><button className="discover-button" onClick={discover} disabled={busy}><span className="button-dot" />{busy ? 'Discovering…' : 'Discover opportunities'}</button></div>
