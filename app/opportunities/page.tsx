...
export default async function OpportunitiesPage() {
  const { analyses } = await getOverview();
  const rows = analyses.filter(a => a.is_problem).sort((a,b) => Number(b.opportunity_score||0)-Number(a.opportunity_score||0));
  const top = rows[0];
  return <AppShell active="Opportunities"><div className="content">
    ...
    <p className="lead">Ranked by pain, demand, willingness to pay, evidence quality, urgency, workarounds and competitive pressure for the latest research run.</p>
