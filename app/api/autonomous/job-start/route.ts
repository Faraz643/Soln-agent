...
export async function POST(request: NextRequest) {
  ...
  const c = db();
  const { data: active } = await c.from('agent_runs').select('id,status,topic,metadata,started_at').eq('status', 'running').order('started_at', { ascending: false }).limit(1).maybeSingle();
  ...
  const { data: run, error } = await c.from('agent_runs').insert({
    kind: 'discovery_cycle', status: 'running', topic: topic.topic,
    metadata: { version: 5, autonomous: true, phase: 'discover', source_index: 0, sources, topic_id: topic.id, analyzed_count: 0, clustered: false, enriched: [] },
  }).select('id').single();
  ...
  await c.from('agent_runs').update({ metadata: { version: 5, autonomous: true, phase: 'discover', source_index: 0, sources, topic_id: topic.id, master_discovery_run_id: master.id, analyzed_count: 0, clustered: false, enriched: [] } }).eq('id', run.id);
