...
async function analyzeOne(doc: any, c: ReturnType<typeof db>, researchTopic: string | null) {
  try {
    ...
    // Relevance is a hard gate. An unrelated document must never become a
    // problem/opportunity just because its AI scores happen to be high.
    const topicRejected = ai.topic_relevance_score < 60;
    const problemRejected = !ai.is_problem;
    const status = topicRejected || problemRejected
      ? 'rejected'
      : ai.confidence_score < 45
        ? 'candidate'
        : ai.opportunity_score >= 75
          ? 'verified'
          : 'candidate';
    ...
    const row = {
      ...
      problem_summary: topicRejected ? null : (ai.problem_summary || null),
      opportunity_summary: topicRejected ? null : (ai.opportunity_summary || null),
      opportunity_score: topicRejected ? 0 : ai.opportunity_score, confidence_score: ai.confidence_score,
      rejection_reason: topicRejected
        ? `Document is not sufficiently relevant to the research topic (${ai.topic_relevance_score}/100): ${ai.topic_relevance_reason || 'content does not materially match the topic'}`
        : status === 'rejected' ? (ai.rejection_reason || 'Not a meaningful problem signal') : null,
      evidence: {
        source, research_topic: topic, metadata: meta, lifecycle: gh,
        topic_relevance: { score: ai.topic_relevance_score, reason: ai.topic_relevance_reason },
        model_scores: { pain: ai.pain_score, demand: ai.demand_score, payment: ai.payment_score, evidence_quality: ai.evidence_quality, urgency: ai.urgency_score, competition: ai.competition_score, workaround: ai.workaround_score },
        customer_segments: ai.customer_segments, ai_evidence: ai.evidence,
      },
      model, analyzed_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      pain_score: ai.pain_score, demand_score: ai.demand_score, payment_score: ai.payment_score,
      evidence_quality: ai.evidence_quality, urgency_score: ai.urgency_score, competition_score: ai.competition_score,
...
export async function POST(request: NextRequest) {
  ...
  if (!runId && !ids.length) {
    const latest = await c.from('discovery_runs').select('id,query').order('started_at', { ascending: false }).limit(1).maybeSingle();
  ...
  if (runId) {
    ...
    if (!error && docs?.length) {
      const meta = json(docs[0].metadata);
      query = String(meta.research_topic || meta.query || '').trim() || null;
    }
  }
