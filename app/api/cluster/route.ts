...
const similarity = (a: any, b: any) => {
  ...
  const segA = new Set((a.customer_segments || []).map((x: string) => String(x).toLowerCase()));
  const segB = new Set((b.customer_segments || []).map((x: string) => String(x).toLowerCase()));
...
function clusterAnalyses(rows: any[]) {
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < clusters.length; i++) {
      const comparisons = clusters[i].map(m => similarity(row, m));
      const match = comparisons.sort((a, b) => b.score - a.score)[0];
      if (match && (match.score > bestScore || (match.segmentOverlap && !bestSegment))) {
        best = i; bestScore = match.score; bestCommon = match.common; bestSegment = match.segmentOverlap;
      }
    }
...
export async function POST(req: NextRequest) {
  ...
  const limit = Math.min(Math.max(Number(b.limit) || 300, 1), 1000);
  const c = db();
  ...
  const rows = analyzed.data || [];
  const rawMap = new Map<string, any>();
  for (const r of rows) rawMap.set(r.raw_document_id, r.raw_documents);
  ...
  for (const list of clusters) {
    ...
    const sourceUrls: { source: string; url: string }[] = [];
    const sourceNames = new Set<string>();
    for (const r of list) {
      const raw = rawMap.get(r.raw_document_id);
      const src = String(r.evidence?.source || raw?.sources?.type || 'unknown');
      sourceNames.add(src);
      if (raw?.url) sourceUrls.push({ source: src, url: String(raw.url) });
      const md = raw?.metadata && typeof raw.metadata === 'object' ? raw.metadata : {};
      const identity = md.username || md.author || md.author_id || (md.original_external_id ? `${src}:${md.original_external_id}` : null);
      if (identity) participants.add(`${src}:${String(identity)}`);
    }
    const participantCount = participants.size || list.length;
    ...
    const metadata = {
      cluster_key: [...tokens(list.map((x: any) => x.problem_summary || '').join(' '))].sort().slice(0, 16).join(' '),
      segments: seg, source_count: sourceCount, source_names: [...sourceNames], evidence_count: list.length,
      participant_count: participantCount, paid_evidence_count: paidEvidenceCount, recent_evidence_count: recentCount,
      source_urls: sourceUrls.slice(0, 30), last_research_run_id: runId, research_query: run.data.query,
      quality_gate: { relevance, confidence, evidence: evidenceQuality, pain, demand, payment, urgency, workaround, competition, opportunity_score: score, recurring, cross_source: crossSource, strong_paid: strongPaid, evidence_count: list.length, participant_count: participantCount, paid_evidence_count: paidEvidenceCount },
    };
    ...
    const prior = new Date(start.getTime() - 30 * 86400000);
