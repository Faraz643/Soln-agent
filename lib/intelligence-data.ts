...
export async function getOpportunities(limit=100){const c=db();if(!c)return[];const {data}=await c.from('opportunities').select('*,problems(*)').order('score',{ascending:false}).limit(limit);return(data||[]).filter((x:any)=>x?.metadata?.gate==='qualified')}
...
async function getLatestRunScoped(c:any, limit:number){
  ...
  const storedOpportunities=storedProblems.length?((await c.from('opportunities').select('*,problems(*)').in('problem_id',storedProblems.map((x:any)=>x.id)).order('score',{ascending:false})).data||[]).filter((x:any)=>x?.metadata?.gate==='qualified'):[];
...
export async function getOverview(){
  ...
  if(scoped){
    ...
    const opportunities=storedOpportunities.length;
